...
-- todo: figure out how to remove so much dependence on control key
-- maybe bind capslock held to control pressed to escape (bit brittle seeming)
...
-- code
set("n", "K", vim.lsp.buf.hover, { buffer = 0, desc = "open lsp hover popup" })      -- todo: better desc for this
...
-- basically no point to this i don't think
-- set("n", "<leader>ft", builtin.git_files, { desc = "[f]ind gi[t] files"}) -- todo: cringe

-- set("n", "<leader>gw", builtin.grep_string, { desc = "[g]wep st[w]ing"}) -- todo: this is useful but needs better key
...
set("n", "<leader>fS", builtin.lsp_workspace_symbols, { desc = "[f]ind workspace [S]ymbols" })
...
vim.keymap.set("n", "<leader>db", dap.toggle_breakpoint, { desc = "add [d]ebugger [b]reakpoint" })

-- todo: debug (<leader>d)
-- todo: requires config to run!
-- vim.keymap.set("n", "<leader>dr", dap.run, { desc = "[d]ebug [r]un" })
-- dap.up
-- vim.keymap.set("n", "<leader>dr", dap.run, { desc = "[d]ebug [r]un" })
-- doesn't work without active session
-- todo: need to copy some working debug configuration
-- & figure this out
